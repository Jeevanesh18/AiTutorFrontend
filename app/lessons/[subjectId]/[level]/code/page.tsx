...
interface CodingQuestion {
  ...
  levelId: number;
  prompt: string;
  starterCode: string; // This is now a JSON string representing an array of files
  ...
  fetch(`http://localhost:8080/api/codingQuestion/level/${levelId}`, {
    ...
    .then((data: CodingQuestion[]) => {
      ...
      const filesArray: CodeFile[][] = data.map(q => {
        ...
        console.log("Raw starter code:", rawStarter);
        try {
          const json = JSON.parse(rawStarter);
          console.log("Parsed starter code JSON:", json);
          if (Array.isArray(json)) {
            ...
            console.log("Parsed starter code JSON failed, using plain text.");
          // Fallback for legacy plain text starter code
        }
                    console.log("Using raw starter code as single file.");
        return [{ name: "Main", code: rawStarter }];
      });

      setAllFiles(filesArray); // Set state directly
      setActiveTabs(new Array(data.length).fill(0));
      console.log("Initialized allFiles:", filesArray);
    });
}, [session, levelId]);
...
const safeSend = (ws: WebSocket | null, data: any) => {
  ...
  //should be changing the questions, when the currentIndex changes
  //will come back later to fix
  useEffect(() => {
   if (!questions.length) return;
            ...
            editor.setValue(allFiles[currentIndex][activeTabs[currentIndex]].code || "", -1);
            editor.setShowPrintMargin(false);

            editorRef.current = editor;

           // Cleanup: Destroy the instance, but DO NOT remove the container div
             return () => {
...
useEffect(() => {
 ...
 useEffect(() => {
  ...
  const term = new Terminal({
    cursorBlink: true,
    convertEol: true,
    theme: { background: "#0f0f0f", foreground: "#ffffff" },
  });

  // Attach input handler immediately
 term.onData((data) => {
  if (data === "\r") {
...
useEffect(() => {
    ...
    ws.onmessage = (evt) => {
      ...
      if (msg.type === "submit") {
        ...
        if (result.details?.[0]) {
           ...
           terminalRef.current?.writeln(`\x1b[31m[INPUT USED] ${input}\x1b[0m`);
           terminalRef.current?.writeln(`\x1b[31m[EXPECTED] ${expected}\x1b[0m`);
           terminalRef.current?.writeln(`\x1b[31m[ACTUAL]   ${actual}\x1b[0m`);
  ...
  //reset the chat and close the ws on unmount
useEffect(() => {
  return () => {
...
const resetChat = async () => {
  ...
  // Helper to format code as Map<String, String> for backend
  const getCodePayload = (currentIndex:number) => {
    const codeMap: { [key: string]: string } = {};
    allFiles[currentIndex].forEach(f => {
  ...
  useEffect(() => {
    if (completed.length > 0 && completed.every(Boolean)) {
      ...
      // 2. Send API Request
      // Assuming session.user has an id. If using standard NextAuth with JWT,
      // you might need to cast or ensure the id is in the session object.
      // Here we assume session.user.id exists as per requirements.
      ...
      if(userId) {
          fetch(`http://localhost:8080/api/levelCompletion/complete-quiz`, {
            ...
            body: JSON.stringify({
              ...
              if(res.ok) console.log("Level completion recorded.");
              else console.error("Failed to record completion");
...
useEffect(() => {
  ...
  /* ================= UI ================= */
  return (
    <div className="flex h-screen gap-4 p-4 bg-gray-900 text-white overflow-hidden">
      
      {/* 1. LEFT SIDEBAR: PROGRESS (VERTICAL) */}
      <div className="w-1/6 min-w-[100px] max-w-[150px] bg-gray-800 rounded-lg p-4 border border-gray-700 flex flex-col">
        <h3 className="font-bold mb-3 text-lg text-center">Progress</h3>
        {/* Changed from grid to flex-col for vertical alignment */}
        <div className="flex flex-col gap-3 overflow-y-auto pr-1 items-center">
          {questions.map((q, i) => {
            ...
            return (
              <button
                key={q.id}
                className={`w-12 h-12 rounded-md text-lg font-semibold text-white transition-all ${bgClass} ${ringClass}`}
      ...
      {/* 2. CENTER PANEL: EDITOR & TERMINAL */}
      <div className="flex-1 bg-gray-800 rounded-lg p-4 border border-gray-700 flex flex-col min-w-[400px]">
        {/* Question Prompt */}
        <div className="mb-4">
          <h2 className="text-xl font-bold mb-2 text-blue-400">Question {currentIndex + 1}</h2>
          <div className="bg-gray-900 p-3 rounded border border-gray-700 text-gray-200 text-sm max-h-[100px] overflow-y-auto">
            {questions[currentIndex]?.prompt || "Loading..."}
          </div>
        </div>

        {/* TABS for Multi-class */}
        <div className="flex gap-1 mb-0 border-b border-gray-600">
            {allFiles[currentIndex]?.map( (file, idx) => (
                <button
                    key={idx}
                    onClick={() => setActiveTabs(prev => {prev[currentIndex] = idx; return [...prev];})}
                    className={`px-4 py-2 text-sm font-medium rounded-t-md transition-colors ${
                        activeTabs[currentIndex] === idx 
                        ? "bg-gray-700 text-blue-400 border-t border-l border-r border-gray-600" 
        ...
        {/* Code Editor */}
        <div className="flex-1 flex flex-col relative border border-gray-600 rounded-b overflow-hidden bg-gray-900">
          <div id="editor" className="flex-1 w-full" style={{ minHeight: "200px" }} />
        </div>
        ...
        <div className="flex gap-3 my-3">
          <button 
            onClick={runCode}
            className="px-4 py-2 bg-blue-600 rounded font-semibold hover:bg-blue-700 transition text-sm"
          >
            Run Code
          </button>
          <button 
            onClick={submitCode}
            className="px-4 py-2 bg-green-600 rounded font-semibold hover:bg-green-700 transition text-sm"
          >
            Submit
          </button>
        </div>
        ...
        <div className="h-[200px] flex flex-col">
          <h3 className="text-sm font-bold text-gray-400 mb-1">Console Output</h3>
          <div
            id="terminal-container"
            className="flex-1 bg-black rounded border border-gray-700 overflow-hidden pl-2 pt-2"
          />
        </div>
      </div>

      {/* 3. RIGHT SIDEBAR: AI CHAT */}
      <div className="w-1/4 min-w-[250px] bg-gray-800 rounded-lg p-4 border border-gray-700 flex flex-col">
        <h3 className="font-bold mb-3 text-lg flex justify-between items-center">
          AI Assistant
          <span className="text-xs font-normal text-gray-400 bg-gray-700 px-2 py-1 rounded">Beta</span>
        </h3>
        
        <AIChat
          question={questions[currentIndex]}
          files={allFiles[currentIndex]} // Passed all files
...
function AIChat({
  ...
  const send = async () => {
    ...
    // Prepare code string from all classes
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto bg-gray-900 border border-gray-700 rounded p-3 mb-3 space-y-3"
      >
        {msgs.length === 0 && (
          <p className="text-gray-500 text-sm text-center mt-10">Ask me for help with your code!</p>
        )}
        
        {msgs.map((m, i) => (
          <div
            key={i}
            className={`p-2 rounded text-sm max-w-[90%] break-words ${
              m.role === "user"
        ...
        {loading && <div className="text-gray-500 text-xs italic">AI is thinking...</div>}
      </div>
      ...
      <div className="flex flex-col gap-2">
        <textarea
          ...
          rows={2}
          className="w-full resize-none bg-gray-900 border border-gray-600 rounded p-2 text-sm focus:outline-none focus:border-blue-500 transition"
          onKeyDown={(e) => {
        ...
        <button 
            onClick={send} 
            disabled={loading}
            className="w-full py-2 bg-blue-600 rounded text-sm font-semibold hover:bg-blue-700 disabled:bg-gray-600 transition"
        >
...
//             return copy;
//           });
//           console.log("Marked question", currentIndex, "as completed.");
...
//         <h3 className="font-bold mb-3 text-lg flex justify-between items-center pb-2 border-b border-gray-600">
//             AI Assistant
//         </h3>
//         <AIChat
//           question={questions[currentIndex]}
